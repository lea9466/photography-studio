/**
 * One-time rollout of the private-galleries feature announcement email,
 * split into 3 groups (one group per day) to stay under the sending
 * provider's ~100/day quota. Same bookkeeping model as
 * send-custom-domain-announcement.ts: state lives in a git-tracked JSON
 ...
 *
 * PRECONDITIONS before running "send":
 *   1. `CLIENT_GALLERIES_ENABLED` + `DOWNLOAD_PERMISSIONS_ENABLED`
 *      (lib/types/app.types.ts) are `true` and deployed — the client-gallery
 *      workspace and downloads are what the email announces.
 *   2. `PRIVATE_GALLERY_CHECKOUT_ENABLED=true` in the environment so the
 *      Starter/Pro/Unlimited upgrade buttons are live, and one real
 ...
 *   npx tsx --env-file=.env.local scripts/send-private-galleries-announcement.ts send <1|2|3> [--dry-run]
 *
...
import { sendPrivateGalleriesAnnouncementEmail } from '../lib/email/resend.ts'

const __dirname = dirname(fileURLToPath(import.meta.url))
const STATE_PATH = join(__dirname, 'data', 'private-galleries-announcement-state.json')
...
async function sendGroup(group: 1 | 2 | 3, dryRun: boolean) {
  ...
  for (const row of rows) {
    ...
    try {
      await sendPrivateGalleriesAnnouncementEmail({
