 ...
 *   2. delete unpaid draft pass galleries abandoned at checkout — uploads are
 *      blocked before payment so these have no photos / R2 objects, and
 *      gallery_settings/selections cascade;
...
export async function runGalleryPassLifecycle() {
  ...
  // 2. Delete abandoned unpaid drafts.
  ...
  if (abandonedIds.length > 0) {
    await admin.from('galleries').delete().in('id', abandonedIds)
  }
