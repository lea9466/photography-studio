...
import { ModernBlogShareLinks } from './ModernBlogShareLinks'
import type { SiteLanguage } from '@/lib/site-language'
import { isSiteLtr } from '@/lib/site-language'
import styles from './ModernBlogPostPage.module.css'
...
/** Local prop shape — mirrors ClassicBlogPostPageItem; see that type's doc
 * comment for why `content` renders as plain JSX text (never
...
export type ModernBlogPostPageItem = {
  id: string
  title: string
...
export type ModernBlogPostNavItem = {
  id: string
  title: string
  coverUrl: string | null
  href: string
...
export type ModernBlogPostPageProps = {
  ...
  postPath: string
  prevPost: ModernBlogPostNavItem | null
  nextPost: ModernBlogPostNavItem | null
}

/**
 * Modern-theme single blog post page — same shape as
 * ClassicBlogPostPage.tsx (full-bleed cover hero with the title overlaid at
 * the bottom when there's a cover image, or a centered title/subtitle header
 * when there isn't; date, share row, body content, extra post images, then a
 * prev/next post nav), restyled with modern's own tokens.
 */
export function ModernBlogPostPage(props: ModernBlogPostPageProps) {
  const {
    accentColor,
    language,
    post,
  ...
  // markup either way). Mirrors ClassicBlogPostPage.tsx.
    ...
    <main className={hasCover ? styles.mainWithHero : styles.mainNoHero}>
      ...
      <section className={styles.content} dir={contentDir}>
        <article>
          ...
          {/* Plain text, not markup — see ModernBlogPostPageItem's doc comment above. */}
