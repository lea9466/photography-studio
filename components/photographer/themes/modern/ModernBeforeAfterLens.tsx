...
import styles from './ModernBeforeAfterLens.module.css'

export type ModernBeforeAfterLensReadyState = {
  ready: boolean
...
export type ModernBeforeAfterLensProps = {
  ...
  onReadyChange: (state: ModernBeforeAfterLensReadyState) => void
}
...
/**
 * Modern-theme "development" (reveal-lens) display style — identical pointer/
 * keyboard state machine to ClassicBeforeAfterLens.tsx (a circular lens that
 * follows the pointer, clipping the original image into view where it
 * hovers; the touch-drag-vs-vertical-scroll disambiguation via an 8px-move
 * threshold; keyboard arrow-key nudging) — see that component's doc comment
 * for the full rationale. Only the CSS module differs (modern's TOKENS
 * values instead of classic's).
 */
export function ModernBeforeAfterLens(props: ModernBeforeAfterLensProps) {
  const {
    originalImageUrl,
    editedImageUrl,
    editedAlt,
