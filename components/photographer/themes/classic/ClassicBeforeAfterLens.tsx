...
import styles from './ClassicBeforeAfterLens.module.css'

export type ClassicBeforeAfterLensReadyState = {
  ready: boolean
...
export type ClassicBeforeAfterLensProps = {
  ...
  onReadyChange: (state: ClassicBeforeAfterLensReadyState) => void
}
...
/**
 * React port of the "development" (reveal-lens) display style — a circular
 * lens that follows the pointer, clipping the original image into view
 * where it hovers. Ported from REVEAL_LENS_CSS / REVEAL_LENS_SCRIPT in
 * lib/public-before-after-html.ts.
 *
 * The old script juggled a `pending` pointer-down record to disambiguate a
 * touch drag (reveal) from a vertical scroll (let the page scroll) by
 * waiting for an 8px move and checking which axis dominates — that's kept
 * here via a plain ref (it must survive across pointermove calls without
 * forcing a re-render on every pixel of movement). Pointer capture and
 * getBoundingClientRect() are both read only from this component's own
 * `frameRef` node, never via document-wide queries.
 */
export function ClassicBeforeAfterLens(props: ClassicBeforeAfterLensProps) {
  const {
    originalImageUrl,
    editedImageUrl,
    editedAlt,
