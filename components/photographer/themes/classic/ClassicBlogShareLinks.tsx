...
import styles from './ClassicBlogShareLinks.module.css'
...
export type ClassicBlogShareLinksProps = {
  shareUrl: string
  postTitle: string
  accentColor: string
...
/**
 * Classic-theme post share row — port of generateBlogShareLinks()
 * (lib/public-blog-html.ts line ~954): copy-link / WhatsApp / email. The
 * copy-link button's "copied!" flash used to be driven by a global
 * `document.addEventListener('click', ...)` delegate
 * (BLOG_SHARE_COPY_SCRIPT) that found its target via `.closest()` — here
 * it's just local component state on the one button this component owns, no
 * DOM querying needed.
 */
export function ClassicBlogShareLinks({ shareUrl, postTitle, accentColor, language }: ClassicBlogShareLinksProps) {
