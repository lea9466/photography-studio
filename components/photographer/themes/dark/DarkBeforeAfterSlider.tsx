...
import styles from './DarkBeforeAfterSlider.module.css'

export type DarkBeforeAfterSliderProps = {
...
/**
 * Dark-theme "split_slider" display style — same mechanics as
 * Classic/ModernBeforeAfterSlider.tsx (a draggable vertical divider driven by
 * a fully-transparent native `<input type="range">` stretched over the whole
 * frame, so dragging AND keyboard arrows both come for free from native
 * range semantics — see those components' doc comments). Only the CSS
 * module differs (dark's TOKENS values instead of another theme's).
 */
export function DarkBeforeAfterSlider(props: DarkBeforeAfterSliderProps) {
  const {
    originalImageUrl,
    editedImageUrl,
    editedAlt,
