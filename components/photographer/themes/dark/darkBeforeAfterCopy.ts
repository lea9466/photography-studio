 ...
 * classic/modernBeforeAfterCopy.ts port, kept local to
 * components/photographer/themes/dark/ per the "define local prop types,
 * don't import old lib types" convention for this rebuild (see those files'
 * doc comments).
 */
export type DarkBeforeAfterDisplayStyle = 'development' | 'split_slider'

export type DarkBeforeAfterCopy = {
  eyebrow: string
  tag: string
  howTo: string
  lensLabel: string
...
export function getDarkBeforeAfterCopy(language: SiteLanguage): DarkBeforeAfterCopy {
