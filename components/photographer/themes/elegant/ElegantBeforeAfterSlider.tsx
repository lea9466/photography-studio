...
import styles from './ElegantBeforeAfterSlider.module.css'

export type ElegantBeforeAfterSliderProps = {
...
/**
 * Elegant-theme "split_slider" display style — same mechanics as
 * Classic/Dark/ModernBeforeAfterSlider.tsx (a draggable vertical divider
 * driven by a fully-transparent native `<input type="range">` stretched over
 * the whole frame, so dragging AND keyboard arrows both come for free from
 * native range semantics — see those components' doc comments). Only the
 * CSS module differs (elegant's TOKENS values instead of another theme's).
 */
export function ElegantBeforeAfterSlider(props: ElegantBeforeAfterSliderProps) {
  const {
    originalImageUrl,
    editedImageUrl,
    editedAlt,
