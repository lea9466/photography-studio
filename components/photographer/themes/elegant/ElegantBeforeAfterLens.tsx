...
import styles from './ElegantBeforeAfterLens.module.css'

export type ElegantBeforeAfterLensReadyState = {
  ready: boolean
...
export type ElegantBeforeAfterLensProps = {
  ...
  onReadyChange: (state: ElegantBeforeAfterLensReadyState) => void
}
...
/**
 * Elegant-theme "development" (reveal-lens) display style — identical
 * pointer/keyboard state machine to Classic/Dark/ModernBeforeAfterLens.tsx (a
 * circular lens that follows the pointer, clipping the original image into
 * view where it hovers; the touch-drag-vs-vertical-scroll disambiguation via
 * an 8px-move threshold; keyboard arrow-key nudging) — reused verbatim per
 * this session's established convention (pure interaction behavior, not
 * theme-specific — see ModernBeforeAfterLens.tsx's doc comment for the same
 * call, and ClassicBeforeAfterLens.tsx as the original source of this logic).
 * Only the CSS module differs (elegant's TOKENS values).
 */
export function ElegantBeforeAfterLens(props: ElegantBeforeAfterLensProps) {
  const {
    originalImageUrl,
    editedImageUrl,
    editedAlt,
