...
- **שעון 60 הימים למחיקה רץ בנפרד** — גלריה מושהית עדיין נמחקת בזמן שלה.

### ו. פאס בודד
- החלון = שליחה + `validity_days` מהחבילה. אחריו הלקוח חסום (כמו היום).
- מחיקה אוטומטית ב‑`expires_at` + 60 יום.
