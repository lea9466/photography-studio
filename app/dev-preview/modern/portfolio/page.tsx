...
import { ModernPortfolioPage } from '@/components/photographer/themes/modern/ModernPortfolioPage'
import type { ModernPortfolioPhoto } from '@/components/photographer/themes/modern/ModernPortfolioGrid'

/**
 * Throwaway preview route for the modern-theme standalone Portfolio page —
 * see the homepage preview route's docblock (app/dev-preview/modern/
 * page.tsx) for why this exists and why it's safe to delete once real
 * page.tsx wiring replaces it. Mirrors app/dev-preview/classic/portfolio/
 * page.tsx's exact mock shape.
 */
...
const MOCK_PHOTOS: ModernPortfolioPhoto[] = MOCK_GALLERY_DEFS.flatMap((gallery) =>
  Array.from({ length: gallery.count }, (_, i) => {
    const height = MOCK_HEIGHTS[(gallery.id.charCodeAt(1) + i) % MOCK_HEIGHTS.length]
    return {
      id: `${gallery.id}-${i}`,
      url: `https://picsum.photos/seed/modern-portfolio-${gallery.id}-${i}/900/${height}`,
...
export default function ModernPortfolioPreviewPage() {
  return (
    <ModernPortfolioPage
