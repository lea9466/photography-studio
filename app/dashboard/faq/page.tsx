...
export default async function FaqPage() {
  ...
  const { data, error } = await supabase
    .from('users')
    .select('faq_items, faq_section_image_url, selected_theme')
    .eq('id', userId)
    .maybeSingle<{
      faq_items: unknown
  ...
  const faqSectionImageUrl = await resolveBrandingPath(data?.faq_section_image_url ?? null)
  const selectedTheme = data?.selected_theme ?? 'elegant'
  const showSectionImageUpload = selectedTheme === 'classic' || selectedTheme === 'elegant'
    ...
    <div className="animate-fade-in">
      <div className="mx-auto max-w-5xl space-y-10 px-6 py-8 md:px-10 md:py-12">
        ...
        <FaqItemsForm
          ...
          initialSectionImageUrl={faqSectionImageUrl}
          showSectionImageUpload={showSectionImageUpload}
        />
      </div>
    </div>
  )
